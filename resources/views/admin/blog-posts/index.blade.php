...
@section('content')
    <div class="space-y-6">
            ...
            <div class="relative px-6 py-7 sm:px-8">
                ...
                <div class="relative flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
                    <div class="max-w-3xl">
                        ...
                        <h1 class="mt-5 text-3xl font-black tracking-normal sm:text-4xl">Manage educational articles and posts.</h1>
                        <p class="mt-3 max-w-2xl text-sm leading-7 text-teal-50/[0.72]">
                            Create, edit, publish, archive, and reorder blog posts for the public website.
                        </p>
                    </div>

                    <div class="flex flex-col gap-3 sm:flex-row">
                        <a href="{{ route('admin.blog.create') }}" class="inline-flex items-center justify-center rounded-md bg-teal-400 px-5 py-3 text-sm font-bold text-[#031a19] shadow-[0_14px_28px_rgba(45,212,191,0.22)] transition hover:bg-teal-300">
                            Add Post
                        </a>
                    </div>
                </div>
        ...
        <section class="grid gap-5 sm:grid-cols-2 lg:grid-cols-5">
            <a href="{{ route('admin.blog.index') }}" class="rounded-lg border border-teal-100 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-teal-200 hover:shadow-md">
                <p class="text-sm font-semibold text-slate-500">All posts</p>
            ...
            @foreach ($statuses as $value => $label)
                ...
                <a href="{{ route('admin.blog.index', ['status' => $value]) }}" class="rounded-lg border border-teal-100 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-teal-200 hover:shadow-md">
        ...
        <section>
            <div class="rounded-lg border border-teal-100 bg-white p-6 shadow-sm">
                <div class="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
                    <div>
                        <p class="text-xs font-bold uppercase tracking-[0.2em] text-teal-700">Manage Posts</p>
                        <h2 class="mt-2 text-2xl font-black tracking-normal text-slate-950">Blog queue</h2>
                    </div>
                    <form method="GET" action="{{ route('admin.blog.index') }}" class="flex flex-col gap-2 sm:flex-row">
                        ...
                        <input name="search" type="search" value="{{ request('search') }}" class="rounded-full border border-teal-100 bg-white px-4 py-2.5 text-sm text-slate-900 shadow-sm placeholder:text-slate-400 focus:border-teal-400 focus:ring-teal-400 sm:w-72" placeholder="Search posts...">
                        ...
                        <tbody class="divide-y divide-slate-100 bg-white text-slate-700">
                            @forelse ($posts as $post)
                                <tr class="transition hover:bg-teal-50/50">
                                    <td class="px-4 py-4 font-semibold text-slate-900">{{ $post->sort_order }}</td>
                                    <td class="px-4 py-4">
                                        <div class="font-semibold text-slate-900">{{ $post->title }}</div>
                                        <p class="mt-1 max-w-xl truncate text-xs font-medium text-slate-500">
                                            {{ $post->slug }}
                                            @if ($post->excerpt)
                                                · {{ \Illuminate\Support\Str::limit(strip_tags($post->excerpt), 70) }}
                                            @endif
                                        </p>
                                    </td>
                                    <td class="px-4 py-4">
                                        <span class="rounded-full px-2.5 py-1 text-xs font-bold {{ $statusClasses[$post->status] ?? 'bg-slate-100 text-slate-600' }}">
                                            {{ $statuses[$post->status] ?? ucfirst($post->status) }}
                                        </span>
                                    </td>
                                    <td class="px-4 py-4 text-slate-500">{{ $post->updated_at?->diffForHumans() }}</td>
                                    <td class="px-4 py-4">
                                        <div class="flex flex-wrap justify-end gap-2">
                                            <a href="{{ route('admin.blog.edit', $post) }}" aria-label="Edit" title="Edit" class="group relative inline-flex size-9 items-center justify-center rounded-md border border-teal-100 bg-white text-teal-700 transition hover:bg-teal-50 hover:text-teal-800 focus:outline-none focus:ring-2 focus:ring-teal-300">
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="px-4 py-12 text-center">
                                        <p class="text-base font-bold text-slate-900">No blog posts yet</p>
                                        <p class="mt-1 text-sm text-slate-500">Create your first educational article.</p>
                                        <a href="{{ route('admin.blog.create') }}" class="mt-4 inline-flex items-center justify-center rounded-md bg-teal-400 px-5 py-2.5 text-sm font-bold text-[#031a19] shadow-[0_14px_28px_rgba(45,212,191,0.22)] transition hover:bg-teal-300">
                                            Add Post
                                        </a>
                                    </td>
                                </tr>
